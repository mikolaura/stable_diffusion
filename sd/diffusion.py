...
class UNET_ResidualBlock(nn.Module):
    def __init(self, in_channels: int, out_channels: int, n_time=1280):
        super().__init__()
...
class UNET_AttentionBlock(nn.Module):
    def __init__(self, n_head: int, n_embd: int, d_context=768):
        ...
        self.attention_2 = CrossAttention(
            n_head, channels, context_dim=d_context, in_proj_bias=False
        )
...
class Upsample(nn.Module):
    def __init__(self, n_embd: int):
        super().__init__()
        self.conv = nn.Conv2d(n_embd, n_embd, kernel_size=3, padding=1)
